...
function setupBlockInteraction(block) {
  ...
  c.on('pointerdown', (e) => {
    ...
    const onTrackMove = (me) => {
      ...
      if (!trackState.hasDragged && (dx * dx + dy * dy) > DRAG_DEAD_ZONE * DRAG_DEAD_ZONE) {
        trackState.hasDragged = true;
        // Start drag (move block)
        dragState = {
    ...
    const onTrackUp = () => {
      ...
      if (!trackState.hasDragged) {
        // It was a tap — check for double-tap to split
        const doubleTapThreshold = (typeof CONFIG.stages !== 'undefined' && CONFIG.stages.split)
          ? CONFIG.stages.split.doubleTapThresholdMs : 400;
        if (now - lastTapTime < doubleTapThreshold && block.value > 1) {
          // Double tap! Split the block
          lastTapTime = 0;
          if (typeof splitBlock === 'function') {
            splitBlock(block);
          }
        } else {
          lastTapTime = now;
...
// ======================== CUSTOM LAYOUT BLOCKS (used by zombie system) ========================
